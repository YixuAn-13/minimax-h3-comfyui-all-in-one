...
# ==========================================
# Legacy node: MinimaxH3LatentUpscalerNode3D
# Reuses the core inference logic via the new-API execute() and unwraps NodeOutput.
# This is the node id referenced by existing Beta2 / community workflows.
# ==========================================
class MinimaxH3LatentUpscalerNode3D:
    """Legacy node (scale mode) used by existing workflows."""
    ...
    def run(self, latent, model_name, scale, device, precision):
        out = MinimaxH3LatentUpscaler3D.execute(
            latent, model_name,
            {"mode": UpscaleMode.SCALE_BY, "scale": scale},
            32, device, precision,
        )
        return (out[0],)
